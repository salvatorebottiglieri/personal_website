...
const HeroChat = ({ onGenerate }: HeroChatProps) => {
  ...
  const visitorRef = useRef<Record<string, string | null> | null>(null);

  useEffect(() => {
    const loadVisitor = async () => {
      const visitor: Record<string, string | null> = {};
      try {
        const res = await fetch(
          "https://ip-api.com/json/?fields=city,region,country,org,isp,query",
        );
        const geo = (await res.json()) as Record<string, string>;
        for (const key of [
          "city",
          "region",
          "country",
          ...
          if (geo[key]) visitor[key] = geo[key];
        }
      } catch {
        /* ip-api failed — ref stays null */
      }

      if (document.referrer) visitor.referrer = document.referrer;
      visitor.timezone =
        Intl.DateTimeFormat().resolvedOptions().timeZone || null;

      if (Object.keys(visitor).length) visitorRef.current = visitor;
    };
    loadVisitor();
  }, []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert(
        "Could not connect to the AI Backend. Ensure it's running on port 8000.",
      );
    } finally {
      setIsLoading(false);
      setInput("");
  ...
  return (
    ...
    >
      <div className="w-full max-w-2xl mx-auto px-4">
        <div className="relative group">
          ...
          <div className="relative bg-secondary ring-1 ring-gray-900/5 rounded-2xl leading-none flex items-top justify-start space-x-6 p-2">
            <form onSubmit={handleSubmit} className="flex-1 flex items-center">
              <Sparkles
                className={`w-6 h-6 text-accent ml-3 mr-4 ${isLoading ? "animate-spin" : "animate-pulse"}`}
              />
              <input
                type="text"
                value={input}
        ...
        <p
          className={`text-center text-gray-500 mt-6 text-sm transition-opacity duration-1000 delay-500 ${isVisible ? "opacity-100" : "opacity-0"}`}
        >
          Press{" "}
          <kbd className="font-mono bg-gray-800 px-2 py-1 rounded text-xs text-gray-400">
            Enter
          </kbd>{" "}
